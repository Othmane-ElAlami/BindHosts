...
ksu_susfs_open_redirect() {
	# Newer susfs v2.1.0 uses uid_scheme as a third argument
	# 2 = Effective for non-su processes
	${SUSFS_BIN} add_open_redirect /system/etc/hosts "$MODDIR/system/etc/hosts" 2 || ${SUSFS_BIN} add_open_redirect /system/etc/hosts "$MODDIR/system/etc/hosts"
...
if [ -w $target_hostsfile ] ; then
	...
	custom_count=$( grep -vEc "0.0.0.0| localhost|#" $target_hostsfile )

	grep -q "# bindhosts v" $target_hostsfile > /dev/null 2>&1 && bindhosts_enabled=true
	grep -q "generated by AdAway" $target_hostsfile > /dev/null 2>&1 && adaway_enabled=true
