  ...
  const fetchStatus = async (item) => {
    try {
      const data = await fetchText(
        "link/MODDIR/" + item.file,
        `${moduleDirectory}/${item.file}`,
      );
      let value = data
        .match(new RegExp(`${item.key}=(.*)`))[1]
        .replace(/^status:\s*/i, "");
      if (item.element === "status-text" && value.length > 0) {
...
async function checkDevOption() {
  ...
  try {
    await fetchText(
      "link/PERSISTENT_DIR/mode_override.sh",
      `${basePath}/mode_override.sh`,
    );
    setDeveloperOption(true);
  } catch {
...
async function saveModeSelection(mode) {
  ...
  if (result.errno === 0) {
    ...
    showPrompt(
      getString("global_reboot_now"),
      true,
      5000,
      getString("global_reboot"),
      reboot,
    );
...
function setupModeBtn() {
  ...
  document.getElementById("reset-mode").onclick = () =>
...
function loadMoreHosts(callback) {
  ...
  for (
    let i = 0;
    i < batchSize && currentIndex < hostLines.length;
    i++, currentIndex++
  ) {
...
async function handleRemove(event, domains) {
  ...
  const result = await exec(
    `sh ${moduleDirectory}/bindhosts.sh --whitelist ${domains.join(" ")}`,
  );
  if (result.errno === 0) {
...
function setupQueryInput() {
  ...
  searchBtn.onclick = () => {
    ...
    const filteredHosts = originalHostLines.filter(([hostIp, ...domains]) => {
      return (
        hostIp.toLowerCase().includes(query) ||
...
export function onHide() {
  ...
  document
    .querySelectorAll(".fab-container")
    .forEach((c) => c.classList.remove("show", "inTerminal"));
}
